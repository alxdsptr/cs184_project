    ...
    PrimaryHitSurfaces gbuffer)
{
    ...
    for (uint32_t bounce = 0; bounce < maxBounces; bounce++) {
        ...
        // Bounded heterogeneous volume integration. See OptiXPrograms.cu and
        // core/VolumeDevice.cuh for the algorithm overview.
        if (scene.medium.enabled && scene.medium.majorantSigmaT > 0.0f) {
                                ...
                                scene.medium, tEnter, tExit))
            {
                ...
                // No scatter — apply ratio-tracked transmittance.
        ...
        if (scene.d_areaLights && scene.areaLightCount > 0 &&
            ...
            if (NdotL > 0.0f && lightNdot > 0.0f) {
                ...
                float3 volumetricTransmittance = volumeShadowTransmittance(
                    shadowOriginV, Ld, dist, scene.medium, rng);
                shadowTransmittance = shadowTransmittance * volumetricTransmittance;
        ...
        // "no area lights" would drop those entirely.
            ...
            for (uint32_t li = 0; li < scene.pointLightCount; li++) {
                ...
                float3 volumetricTransmittancePL = volumeShadowTransmittance(
                    shadowOriginPL, Ld, dist, scene.medium, rng);
                shadowTransmittancePL = shadowTransmittancePL * volumetricTransmittancePL;
