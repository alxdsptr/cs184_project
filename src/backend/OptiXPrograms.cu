...
// in core/VolumeDevice.cuh. Use phaseHGEval / phaseHGSample / volumeDeltaTrack
// / volumeRatioTrack / volumeShadowTransmittance from there.
...
extern "C" __global__ void __raygen__path_trace()
{
    ...
    for (uint32_t s = 0; s < samplesPerPixel; s++) {
        ...
        for (uint32_t bounce = 0; bounce < maxBounces; bounce++) {
            ...
            // Per-segment volume integration. Three exit paths:
            //   • scatter event found inside the bounded volume → NEE +
            //     phase sampling, restart bounce loop from the scatter point;
            //   • no scatter inside the volume → throughput multiplied by
            //     ratio-tracked transmittance, fall through to surface shading;
            //   • medium disabled / segment misses bounds → no work.
            if (scene.medium.enabled && scene.medium.majorantSigmaT > 0.0f) {
                                    ...
                                    scene.medium, tEnter, tExit))
                {
                    ...
                    if (scattered) {
                        ...
                        // σ_s/σ_t single-scatter albedo at the scatter point.
                        // For our uniform-σ-with-density model this is the
                        // same constant everywhere, so density never appears.
                    ...
                    // transmittance for the inside-volume span. The before/
                    // after-volume spans are vacuum, so they contribute 1.
            ...
            // whether area lights also exist. See PathTraceKernel.cu comment.
                ...
                for (uint32_t li = 0; li < scene.pointLightCount; li++) {
                    ...
                    float3 volumetricShadowTPL = volumeShadowTransmittance(
                        shadowOrigin, Ld, dist, scene.medium, rng);
                    shadowTransmittancePL = shadowTransmittancePL * volumetricShadowTPL;
            ...
            if (scene.d_directionalLights && scene.directionalLightCount > 0) {
                ...
                for (uint32_t li = 0; li < scene.directionalLightCount; li++) {
                    ...
                    float3 volumetricShadowTDL = volumeShadowTransmittance(
                        shadowOriginDL, Ld, 1e30f, scene.medium, rng);
                    shadowTransmittanceDL = shadowTransmittanceDL * volumetricShadowTDL;
...
extern "C" __global__ void __raygen__path_trace_split()
{
    ...
    for (uint32_t s = 0; s < samplesPerPixel; s++) {
        ...
        for (uint32_t bounce = 0; bounce < maxBounces; bounce++) {
            ...
            if (segmentDistance > 0.0f && scene.medium.enabled &&
                scene.medium.majorantSigmaT > 0.0f)
            {
                float tEnter, tExit;
